...
describe("voidJournal()", () => {
  it("voids a DRAFT journal", async () => {
    ...
    const result = await voidJournal({ tenantId: "t1", journalId: IDS.journal, userId: "u1", reason: "entered in error" }, deps);
  ...
  it("rejects POSTED journals", async () => {
    ...
    const deps = makeDeps({ journalRepo: mockJournalRepo(new Map([[IDS.journal, journal]])) });
    const result = await voidJournal({ tenantId: "t1", journalId: IDS.journal, userId: "u1", reason: "test" }, deps);
  ...
  it("returns NOT_FOUND for missing journal", async () => {
    const deps = makeDeps();
    const result = await voidJournal({ tenantId: "t1", journalId: "missing", userId: "u1", reason: "test" }, deps);
    expect(result.ok).toBe(false);
  });
  ...
  it("passes reason to audit log (A-11)", async () => {
    ...
    await voidJournal({ tenantId: "t1", journalId: IDS.journal, userId: "u1", reason: "duplicate draft" }, deps);
