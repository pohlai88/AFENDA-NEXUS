...
function makeSection(
  label: string,
  rows: { code: string; name: string; amount: bigint }[]
): ReportSection {
  return {
    label,
    rows: rows.map((r) => ({
...
describe('buildComparativeSection (FR-05 calculator)', () => {
  ...
  it('handles accounts present only in current period', () => {
    ...
    const prior = makeSection('Assets', [{ code: '1000', name: 'Cash', amount: 40000n }]);
  ...
  it('handles accounts present only in prior period', () => {
    const current = makeSection('Assets', [{ code: '1000', name: 'Cash', amount: 50000n }]);
    const prior = makeSection('Assets', [
      { code: '1000', name: 'Cash', amount: 40000n },
  ...
  it('computes variance percent correctly', () => {
    const current = makeSection('Revenue', [{ code: '4000', name: 'Sales', amount: 150000n }]);
    const prior = makeSection('Revenue', [{ code: '4000', name: 'Sales', amount: 100000n }]);
