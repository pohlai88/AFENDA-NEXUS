...
function makeRow(
  overrides: Partial<BudgetVarianceRow> & { code: string; budget: bigint; actual: bigint }
): BudgetVarianceRow {
...
describe('evaluateVarianceAlerts (A-20)', () => {
  it('returns no alerts when variance is within threshold', () => {
    const rows: BudgetVarianceRow[] = [makeRow({ code: '5000', budget: 10000n, actual: 10500n })];
  ...
  it('flags WARNING when variance exceeds percentage warning threshold', () => {
    const rows: BudgetVarianceRow[] = [makeRow({ code: '5000', budget: 10000n, actual: 11500n })];
    const { result } = evaluateVarianceAlerts(rows);
  ...
  it('flags CRITICAL when variance exceeds percentage critical threshold', () => {
    const rows: BudgetVarianceRow[] = [makeRow({ code: '5000', budget: 10000n, actual: 13000n })];
    const { result } = evaluateVarianceAlerts(rows);
    expect(result.alerts).toHaveLength(1);
  ...
  it('supports custom thresholds', () => {
    const rows: BudgetVarianceRow[] = [makeRow({ code: '5000', budget: 10000n, actual: 10600n })];
    const { result } = evaluateVarianceAlerts(rows, {
      percentageWarning: 5,
  ...
  it('supports absolute thresholds', () => {
    const rows: BudgetVarianceRow[] = [makeRow({ code: '5000', budget: 100000n, actual: 100500n })];
    const { result } = evaluateVarianceAlerts(rows, {
  ...
  it('skips zero-budget rows', () => {
    const rows: BudgetVarianceRow[] = [makeRow({ code: '5000', budget: 0n, actual: 5000n })];
    const { result } = evaluateVarianceAlerts(rows);
    expect(result.alerts).toHaveLength(0);
  });
