...
describe("computeEliminations", () => {
  it("produces paired debit+credit entries for each IC balance", () => {
    const balances: IntercompanyBalance[] = [
      { fromCompanyId: "c1", toCompanyId: "c2", accountId: "a1", amountMinor: 5000n, currency: "USD" },
    ];
  ...
  it("handles multiple IC balances", () => {
    const balances: IntercompanyBalance[] = [
      { fromCompanyId: "c1", toCompanyId: "c2", accountId: "a1", amountMinor: 5000n, currency: "USD" },
      { fromCompanyId: "c2", toCompanyId: "c3", accountId: "a2", amountMinor: 3000n, currency: "USD" },
    ];
    const result = computeEliminations(balances);
  ...
  it("throws on negative amount", () => {
    const balances: IntercompanyBalance[] = [
      { fromCompanyId: "c1", toCompanyId: "c2", accountId: "a1", amountMinor: -1n, currency: "USD" },
    ];
  ...
  it("throws on same-company balance", () => {
    const balances: IntercompanyBalance[] = [
      { fromCompanyId: "c1", toCompanyId: "c1", accountId: "a1", amountMinor: 1000n, currency: "USD" },
    ];
...
describe("translateTrialBalance", () => {
  ...
  it("throws on non-positive rates", () => {
    ...
    expect(() =>
      translateTrialBalance(entries, { closingRate: 0, averageRate: 1.4, historicalRate: 1.2 }, "USD"),
    ).toThrow("positive");
  });
});
...
describe("resolveCloseReadiness", () => {
  it("reports ready when all tasks completed", () => {
    const tasks: CloseTask[] = [
      ...
      { id: "t2", name: "Post adjustments", status: "completed", dependsOn: ["t1"], companyId: "c1" },
    ];
