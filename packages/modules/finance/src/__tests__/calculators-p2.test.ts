...
import {
  ...
  getSubtree,
  getAncestors,
} from '../slices/gl/calculators/coa-hierarchy.js';
...
describe('validateCoaIntegrity', () => {
  ...
  it('detects missing parent reference', () => {
    const broken = [makeNode({ id: 'a1', parentId: 'nonexistent' })];
...
describe('getAncestors', () => {
  ...
  it('throws on cycle', () => {
    const cyclic = [makeNode({ id: 'a1', parentId: 'a2' }), makeNode({ id: 'a2', parentId: 'a1' })];
...
describe('validateDimensions', () => {
  ...
  it('validates project dimension', () => {
    const lines: JournalLineDimensions[] = [{ lineId: 'L1', accountId: 'a1', project: 'PRJ-001' }];
    const result = validateDimensions(lines, dims);
    expect(result.result.allValid).toBe(true);
  });

  it('allows lines with no dimensions', () => {
    const lines: JournalLineDimensions[] = [{ lineId: 'L1', accountId: 'a1' }];
    const result = validateDimensions(lines, dims);
    expect(result.result.allValid).toBe(true);
  });
