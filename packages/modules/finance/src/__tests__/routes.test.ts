...
function buildApp(depsOverrides: Partial<FinanceDeps> = {}): {
  ...
  registerAccountRoutes(app, runtime, policy);
  registerPeriodRoutes(app, runtime, policy);
  registerBalanceRoutes(app, runtime, policy);
  registerIcRoutes(app, runtime, policy);
  registerIcAgreementRoutes(app, runtime, policy);
  registerLedgerRoutes(app, runtime, policy);
  registerFxRateRoutes(app, runtime, policy);
  registerRecurringTemplateRoutes(app, runtime, policy);
  registerBudgetRoutes(app, runtime, policy);
  registerReportRoutes(app, runtime, policy);
...
describe('Journal routes', () => {
  ...
  it('GET /journals — 200 returns all journals without periodId filter', async () => {
    const { app: freshApp } = buildApp();
    await freshApp.ready();
    const res = await freshApp.inject({
      method: 'GET',
