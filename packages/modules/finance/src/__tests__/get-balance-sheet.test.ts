...
describe('getBalanceSheet', () => {
  it('returns a balance sheet with asset/liability/equity sections', async () => {
    ...
    const result = await getBalanceSheet({ ledgerId: IDS.ledger, periodId: IDS.period }, deps);
  ...
  it('returns error when ledger not found', async () => {
    ...
    const result = await getBalanceSheet({ ledgerId: 'nonexistent', periodId: IDS.period }, deps);
