...
function mockRevenueContractRepo(contract: RevenueContract = makeContract()): IRevenueContractRepo {
...
describe('recognizeRevenue (AH-04)', () => {
  ...
  it('creates a journal with deferred debit and revenue credit', async () => {
    ...
    const journal = [
      ...(deps.journalRepo as ReturnType<typeof mockJournalRepo>).journals.values(),
    ][0]!;
  ...
  it('updates recognizedToDate on the contract', async () => {
    ...
    expect(deps.revenueContractRepo.updateRecognized).toHaveBeenCalledWith('contract-1', 10000n);
  });
