...
beforeAll(async () => {
  ...
  const periodRows = await db
    .select()
    .from(fiscalPeriods)
    .where(eq(fiscalPeriods.tenantId, TENANT_ID));
  ...
  const [journal] = await db
    .select()
    .from(glJournals)
    .where(eq(glJournals.tenantId, TENANT_ID))
    .limit(1);
...
describe.skipIf(SKIP)('DrizzleJournalRepo (integration)', () => {
  it('creates a journal and reads it back', async () => {
    const result = await session.withTenant(
...
describe.skipIf(SKIP)('DrizzlePeriodRepo (integration)', () => {
  ...
  beforeEach(async () => {
    await db.execute(sql`UPDATE erp.fiscal_period SET status = 'OPEN' WHERE id = ${PERIOD_P02_ID}`);
  });
