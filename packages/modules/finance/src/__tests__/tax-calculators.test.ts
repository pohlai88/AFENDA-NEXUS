...
import { buildSaftFile, validateSaftFile, type SaftHeader } from "../slices/tax/calculators/saft-export.js";
import { computeWhtWithTreaty, type TreatyRate } from "../slices/tax/calculators/wht-treaty.js";
import { computeDeferredTax, type TemporaryDifference } from "../slices/tax/calculators/deferred-tax.js";
import { computeTaxProvision } from "../slices/tax/calculators/tax-provision.js";
import { formatMySst, formatSgGst, formatGenericVat, formatTaxReturn, type TaxReturnData } from "../slices/tax/calculators/country-formats.js";
...
describe("computeVatNetting", () => {
  ...
  it("nets output tax against input tax per jurisdiction", () => {
    const entries: TaxEntry[] = [
      { documentId: "s1", documentType: "SALES_INVOICE", taxCodeId: "tc1", jurisdictionCode: "MY", taxableAmount: 100000n, taxAmount: 6000n, currencyCode: "MYR", documentDate: new Date("2025-02-01") },
      { documentId: "p1", documentType: "PURCHASE_INVOICE", taxCodeId: "tc1", jurisdictionCode: "MY", taxableAmount: 50000n, taxAmount: 3000n, currencyCode: "MYR", documentDate: new Date("2025-02-15") },
    ];
  ...
  it("identifies refundable position when input > output", () => {
    const entries: TaxEntry[] = [
      { documentId: "s1", documentType: "SALES_INVOICE", taxCodeId: "tc1", jurisdictionCode: "MY", taxableAmount: 10000n, taxAmount: 600n, currencyCode: "MYR", documentDate: new Date("2025-02-01") },
      { documentId: "p1", documentType: "PURCHASE_INVOICE", taxCodeId: "tc1", jurisdictionCode: "MY", taxableAmount: 50000n, taxAmount: 3000n, currencyCode: "MYR", documentDate: new Date("2025-02-15") },
    ];
    const result = computeVatNetting(entries, periodStart, periodEnd);
  ...
  it("excludes entries outside period", () => {
    const entries: TaxEntry[] = [
      { documentId: "s1", documentType: "SALES_INVOICE", taxCodeId: "tc1", jurisdictionCode: "MY", taxableAmount: 100000n, taxAmount: 6000n, currencyCode: "MYR", documentDate: new Date("2024-12-01") },
    ];
    const result = computeVatNetting(entries, periodStart, periodEnd);
...
describe("buildSaftFile + validateSaftFile", () => {
  ...
  it("builds valid SAF-T file with balanced debits/credits", () => {
    const file = buildSaftFile(header, [], [
      { transactionId: "tx1", transactionDate: now, description: "Test", documentRef: "J-001", lines: [
        { accountId: "a1", debitAmount: 10000n, creditAmount: 0n, taxCode: null, taxAmount: 0n, currencyCode: "MYR" },
        { accountId: "a2", debitAmount: 0n, creditAmount: 10000n, taxCode: null, taxAmount: 0n, currencyCode: "MYR" },
      ]},
    ], []);
...
describe("computeDeferredTax", () => {
  it("computes DTA and DTL from temporary differences", () => {
    const diffs: TemporaryDifference[] = [
      { itemId: "d1", description: "Depreciation", accountingBase: 100000n, taxBase: 80000n, type: "TAXABLE" },
      { itemId: "d2", description: "Provision", accountingBase: 50000n, taxBase: 70000n, type: "DEDUCTIBLE" },
    ];
  ...
  it("handles all taxable differences", () => {
    const diffs: TemporaryDifference[] = [
      { itemId: "d1", description: "Accel depr", accountingBase: 200000n, taxBase: 100000n, type: "TAXABLE" },
    ];
