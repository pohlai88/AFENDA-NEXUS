...
import { computeOutstandingItems, type OutstandingItem } from "../slices/bank/calculators/outstanding-items.js";
...
describe("autoMatchLines", () => {
  it("matches by amount + date + reference", () => {
    ...
    const candidates: GlCandidate[] = [
      { id: "c1", journalId: "j1", transactionDate: now, amount: 5000n, currencyCode: "USD", reference: "INV-100", description: "Invoice 100" },
    ];
  ...
  it("matches by amount + date only (no reference)", () => {
    ...
    const candidates: GlCandidate[] = [
      { id: "c1", journalId: "j1", transactionDate: now, amount: 5000n, currencyCode: "USD", reference: null, description: "Other desc" },
    ];
    const result = autoMatchLines(lines, candidates);
    expect(result.matched).toHaveLength(1);
  ...
  it("skips already matched lines", () => {
    const lines: BankStatementLine[] = [
      makeLine({ id: "l1", matchStatus: "CONFIRMED" }),
    ];
    const candidates: GlCandidate[] = [
      { id: "c1", journalId: "j1", transactionDate: now, amount: 10000n, currencyCode: "USD", reference: "REF-001", description: "Match" },
    ];
    const result = autoMatchLines(lines, candidates);
    expect(result.matched).toHaveLength(0);
  ...
  it("does not double-match candidates", () => {
    ...
    const candidates: GlCandidate[] = [
      { id: "c1", journalId: "j1", transactionDate: now, amount: 5000n, currencyCode: "USD", reference: null, description: "X" },
    ];
    const result = autoMatchLines(lines, candidates);
    expect(result.matched).toHaveLength(1);
...
describe("computeOutstandingItems", () => {
  it("adjusts bank balance for outstanding checks and deposits", () => {
    const items: OutstandingItem[] = [
      { id: "chk1", documentRef: "CHK-001", documentDate: now, amount: 5000n, currencyCode: "USD", type: "CHECK" },
      { id: "chk2", documentRef: "CHK-002", documentDate: now, amount: 3000n, currencyCode: "USD", type: "CHECK" },
      { id: "dep1", documentRef: "DEP-001", documentDate: now, amount: 10000n, currencyCode: "USD", type: "DEPOSIT" },
    ];
...
describe("computeMultiCurrencyRecon", () => {
  ...
  it("detects FX difference", () => {
    const result = computeMultiCurrencyRecon({
      ...
      baseCurrency: "USD",
      fxRates: [
        { fromCurrency: "EUR", toCurrency: "USD", rateBps: 11000 },
      ],
    });
