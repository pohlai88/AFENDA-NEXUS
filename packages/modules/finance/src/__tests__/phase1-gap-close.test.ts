...
import type { SupplierStatementLine, ApLedgerEntry } from "../slices/ap/calculators/supplier-statement-recon.js";
...
import { computeInvoiceDiscounting, computeBatchDiscounting } from "../slices/ar/calculators/invoice-discounting.js";
import { computeRevenueRecognition, computeBatchRevenueRecognition } from "../slices/ar/calculators/revenue-recognition-hook.js";
...
describe("reconcileSupplierStatement", () => {
  ...
  it("matches statement lines to ledger entries by amount + date", () => {
    const statements: SupplierStatementLine[] = [
      { lineRef: "SL-1", date: new Date("2025-03-01"), description: "Invoice 001", amount: 50000n, currencyCode: "USD" },
    ];
    const ledger: ApLedgerEntry[] = [
      { invoiceId: "inv-1", invoiceNumber: "INV-001", invoiceDate: new Date("2025-03-01"), amount: 50000n, currencyCode: "USD", supplierRef: "SL-1" },
    ];
  ...
  it("identifies statement-only lines", () => {
    const statements: SupplierStatementLine[] = [
      { lineRef: "SL-1", date: new Date("2025-03-01"), description: "Unknown charge", amount: 10000n, currencyCode: "USD" },
    ];
  ...
  it("identifies ledger-only entries", () => {
    const ledger: ApLedgerEntry[] = [
      { invoiceId: "inv-1", invoiceNumber: "INV-001", invoiceDate: new Date("2025-03-01"), amount: 50000n, currencyCode: "USD", supplierRef: null },
    ];
  ...
  it("does not match when amounts differ", () => {
    ...
    const ledger: ApLedgerEntry[] = [
      { invoiceId: "inv-1", invoiceNumber: "INV-001", invoiceDate: new Date("2025-03-01"), amount: 49999n, currencyCode: "USD", supplierRef: null },
    ];
    const result = reconcileSupplierStatement("sup-1", asOf, statements, ledger);
    expect(result.matchedCount).toBe(0);
    expect(result.statementOnlyCount).toBe(1);
  ...
  it("does not match when date exceeds tolerance", () => {
    const statements: SupplierStatementLine[] = [
      { lineRef: "SL-1", date: new Date("2025-03-10"), description: "Invoice 001", amount: 50000n, currencyCode: "USD" },
    ];
    const ledger: ApLedgerEntry[] = [
      { invoiceId: "inv-1", invoiceNumber: "INV-001", invoiceDate: new Date("2025-03-01"), amount: 50000n, currencyCode: "USD", supplierRef: null },
    ];
  ...
  it("computes correct totals and difference", () => {
    const statements: SupplierStatementLine[] = [
      { lineRef: "SL-1", date: new Date("2025-03-01"), description: "A", amount: 30000n, currencyCode: "USD" },
      { lineRef: "SL-2", date: new Date("2025-03-05"), description: "B", amount: 20000n, currencyCode: "USD" },
    ];
    const ledger: ApLedgerEntry[] = [
      { invoiceId: "inv-1", invoiceNumber: "INV-001", invoiceDate: new Date("2025-03-01"), amount: 30000n, currencyCode: "USD", supplierRef: null },
    ];
    const result = reconcileSupplierStatement("sup-1", asOf, statements, ledger);
...
describe("matchIcReceivables", () => {
  it("matches receivables to payables by counterparty + currency", () => {
    const receivables: IcReceivable[] = [
      { invoiceId: "ar-1", customerId: "c1", counterpartyCompanyId: "co-B", amount: 100000n, currencyCode: "USD", invoiceDate: new Date("2025-03-01") },
    ];
    const payables: IcPayable[] = [
      { invoiceId: "ap-1", supplierId: "s1", counterpartyCompanyId: "co-B", amount: 100000n, currencyCode: "USD", invoiceDate: new Date("2025-03-01") },
    ];
  ...
  it("detects amount mismatches", () => {
    ...
    const payables: IcPayable[] = [
      { invoiceId: "ap-1", supplierId: "s1", counterpartyCompanyId: "co-B", amount: 90000n, currencyCode: "USD", invoiceDate: new Date("2025-03-01") },
    ];
    const result = matchIcReceivables(receivables, payables);
  ...
  it("identifies unmatched receivables and payables", () => {
    const receivables: IcReceivable[] = [
      { invoiceId: "ar-1", customerId: "c1", counterpartyCompanyId: "co-B", amount: 50000n, currencyCode: "USD", invoiceDate: new Date("2025-03-01") },
    ];
    const payables: IcPayable[] = [
      { invoiceId: "ap-1", supplierId: "s1", counterpartyCompanyId: "co-C", amount: 30000n, currencyCode: "USD", invoiceDate: new Date("2025-03-01") },
    ];
    const result = matchIcReceivables(receivables, payables);
  ...
  it("does not match different currencies", () => {
    ...
    const payables: IcPayable[] = [
      { invoiceId: "ap-1", supplierId: "s1", counterpartyCompanyId: "co-B", amount: 100000n, currencyCode: "EUR", invoiceDate: new Date("2025-03-01") },
    ];
    const result = matchIcReceivables(receivables, payables);
    expect(result.matchedCount).toBe(0);
  });
...
describe("computeInvoiceDiscounting", () => {
  it("computes discount charge and net proceeds", () => {
    ...
    expect(result.netProceeds).toBe(result.faceValue - result.discountCharge - result.holdbackAmount);
  });
  ...
  it("computeBatchDiscounting handles multiple invoices", () => {
    const results = computeBatchDiscounting([
      { invoiceId: "a", customerId: "c1", faceValue: 500000n, currencyCode: "USD", dueDate: new Date("2025-06-01"), factoringDate: new Date("2025-03-01"), discountRateBps: 300, holdbackBps: 500 },
      { invoiceId: "b", customerId: "c1", faceValue: 300000n, currencyCode: "USD", dueDate: new Date("2025-07-01"), factoringDate: new Date("2025-03-01"), discountRateBps: 200, holdbackBps: 800 },
    ]);
...
describe("computeRevenueRecognition", () => {
  ...
  it("computeBatchRevenueRecognition handles multiple invoices", () => {
    const results = computeBatchRevenueRecognition([
      { invoiceId: "a", customerId: "c1", totalAmount: 100000n, currencyCode: "USD", invoiceDate: new Date("2025-03-01"), deliveryDate: new Date("2025-03-15"), contractId: null, performanceObligationsMet: true },
      { invoiceId: "b", customerId: "c1", totalAmount: 200000n, currencyCode: "USD", invoiceDate: new Date("2025-03-01"), deliveryDate: null, contractId: null, performanceObligationsMet: false },
    ]);
    expect(results).toHaveLength(2);
