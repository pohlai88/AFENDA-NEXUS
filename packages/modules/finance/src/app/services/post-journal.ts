...
import type { IFxRateRepo } from "../ports/fx-rate-repo.js";
...
export async function postJournal(
  ...
  if (journal.status !== "DRAFT") {
    return err(new AppError("INVALID_STATE", `Journal ${journal.id} is ${journal.status}, expected DRAFT`));
  }
  ...
  if (period.status !== "OPEN") {
    return err(new AppError("INVALID_STATE", `Cannot post to ${period.status} period '${period.name}'`));
  }
  ...
  const foreignLines = journal.lines.filter((l) => l.debit.currency !== ledger.baseCurrency || l.credit.currency !== ledger.baseCurrency);
  for (const line of foreignLines) {
    ...
    if (!fxRateCache.has(lineCurrency)) {
      const rateResult = await deps.fxRateRepo.findRate(lineCurrency, ledger.baseCurrency, journal.date);
      if (!rateResult.ok) {
        return err(
          new AppError(
            "VALIDATION",
  ...
  if (saved.ok) {
    ...
    await deps.idempotencyStore.recordOutcome?.(tenantId, input.idempotencyKey, "POST_JOURNAL", journal.id);
  }
