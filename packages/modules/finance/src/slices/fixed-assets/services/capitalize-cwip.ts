...
export async function capitalizeCwip(
  ...
  if (asset.status !== 'CWIP')
    return err(new ValidationError(`Asset status must be CWIP to capitalize, got ${asset.status}`));
  if (input.finalCost <= 0n) return err(new ValidationError('Final cost must be positive'));
