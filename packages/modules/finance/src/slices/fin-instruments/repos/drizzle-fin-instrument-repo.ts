...
export class DrizzleFinInstrumentRepo implements IFinInstrumentRepo {
  ...
  async findById(id: string): Promise<FinancialInstrument | null> {
    const rows = await this.db.select().from(financialInstruments).where(eq(financialInstruments.id, id)).limit(1);
  ...
  async findByCompany(companyId: string): Promise<readonly FinancialInstrument[]> {
    const rows = await this.db.select().from(financialInstruments).where(eq(financialInstruments.companyId, companyId));
    return rows.map(mapToDomain);
  }

  async create(tenantId: string, input: CreateFinInstrumentInput): Promise<FinancialInstrument> {
    const [row] = await this.db.insert(financialInstruments).values({ tenantId, ...input }).returning();
    return mapToDomain(row!);
  }

  async updateClassification(id: string, classification: InstrumentClassification): Promise<FinancialInstrument> {
    const [row] = await this.db.update(financialInstruments).set({ classification }).where(eq(financialInstruments.id, id)).returning();
    return mapToDomain(row!);
  }

  async markDerecognized(id: string): Promise<FinancialInstrument> {
    const [row] = await this.db.update(financialInstruments).set({ isDerecognized: true }).where(eq(financialInstruments.id, id)).returning();
    return mapToDomain(row!);
  }
}
