 ...
 */
export function computeEir(
  inputs: readonly EirInput[],
): CalculatorResult<readonly EirResult[]> {
  if (inputs.length === 0) {
  ...
  const results: EirResult[] = inputs.map((input) => {
    const annualInterest =
      (input.carryingAmount * BigInt(input.effectiveInterestRateBps)) / 10000n;
    const interestIncome =
      (annualInterest * BigInt(input.periodMonths)) / 12n;
