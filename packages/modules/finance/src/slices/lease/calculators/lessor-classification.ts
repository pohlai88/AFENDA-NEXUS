...
export function classifyLessorLease(input: LessorClassificationInput): LessorClassificationResult {
  ...
  const majorPartOfEconomicLife = input.assetEconomicLifeMonths > 0 &&
    (input.leaseTermMonths / input.assetEconomicLifeMonths) * 100 >= MAJOR_PART_THRESHOLD;

  const substantiallyAllFairValue = input.fairValueOfAsset > 0n &&
    (input.pvOfLeasePayments * 100n) / input.fairValueOfAsset >= BigInt(SUBSTANTIALLY_ALL_THRESHOLD);
