...
export function registerApInvoiceRoutes(
  ...
  app.post("/ap/invoices", async (req, reply) => {
  ...
  app.get("/ap/invoices", async (req, reply) => {
    const tenantId = req.headers["x-tenant-id"] as string;
    const userId = (req.headers["x-user-id"] as string) ?? "system";
  ...
  // GET /ap/invoices/:id
  app.get("/ap/invoices/:id", async (req, reply) => {
  ...
  app.post("/ap/invoices/:id/post", async (req, reply) => {
    const { id } = IdParamSchema.parse(req.params);
    const tenantId = req.headers["x-tenant-id"] as string;
    const userId = (req.headers["x-user-id"] as string) ?? "system";
  ...
  app.post("/ap/debit-memos", async (req, reply) => {
    const tenantId = req.headers["x-tenant-id"] as string;
    const userId = (req.headers["x-user-id"] as string) ?? "system";
