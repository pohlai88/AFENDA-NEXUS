...
export class DrizzleGoodwillRepo implements IGoodwillRepo {
  ...
  async findByChild(childEntityId: string): Promise<Goodwill | null> {
    const rows = await this.db.select().from(goodwills).where(eq(goodwills.childEntityId, childEntityId)).limit(1);
    return rows[0] ? mapToDomain(rows[0]) : null;
  }
  ...
  async create(tenantId: string, input: CreateGoodwillInput): Promise<Goodwill> {
    const [row] = await this.db.insert(goodwills).values({
      tenantId,
  ...
  async update(id: string, input: Partial<{
    ...
    if (input.accumulatedImpairment !== undefined) values.accumulatedImpairment = input.accumulatedImpairment;
    if (input.carryingAmount !== undefined) values.carryingAmount = input.carryingAmount;
    if (input.status !== undefined) values.status = input.status;
    const [row] = await this.db.update(goodwills).set(values).where(eq(goodwills.id, id)).returning();
    return mapToDomain(row!);
  }
}
