...
export async function submitExpenseClaim(
  ...
  deps: { expenseClaimRepo: IExpenseClaimRepo; expensePolicyRepo: IExpensePolicyRepo; outboxWriter: IOutboxWriter },
  ...
  if (claim.status !== "DRAFT") return err(new ValidationError("Only draft claims can be submitted"));
  ...
  await deps.outboxWriter.write({
    ...
    payload: { claimId: updated.id, employeeId: updated.employeeId, totalAmount: String(updated.totalAmount), userId: input.userId },
  });
