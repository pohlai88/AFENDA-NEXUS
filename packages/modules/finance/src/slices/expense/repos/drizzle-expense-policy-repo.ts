...
export class DrizzleExpensePolicyRepo implements IExpensePolicyRepo {
  ...
  async findById(id: string): Promise<ExpensePolicy | null> {
    const rows = await this.db.select().from(expensePolicies).where(eq(expensePolicies.id, id)).limit(1);
    return rows[0] ? mapToDomain(rows[0]) : null;
  }

  async findByCompany(companyId: string): Promise<readonly ExpensePolicy[]> {
    const rows = await this.db.select().from(expensePolicies).where(eq(expensePolicies.companyId, companyId));
    return rows.map(mapToDomain);
  }

  async findByCategory(companyId: string, category: string): Promise<ExpensePolicy | null> {
    const rows = await this.db.select().from(expensePolicies).where(
      and(eq(expensePolicies.companyId, companyId), eq(expensePolicies.category, category)),
    ).limit(1);
    return rows[0] ? mapToDomain(rows[0]) : null;
  }
  ...
  async create(tenantId: string, input: CreateExpensePolicyInput): Promise<ExpensePolicy> {
    const [row] = await this.db.insert(expensePolicies).values({
      tenantId,
  ...
  async update(id: string, input: Partial<Record<string, unknown>>): Promise<ExpensePolicy> {
    const [row] = await this.db.update(expensePolicies).set(input).where(eq(expensePolicies.id, id)).returning();
    return mapToDomain(row!);
  }
}
