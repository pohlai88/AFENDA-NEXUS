...
import type { IExpenseClaimRepo, CreateExpenseClaimInput, CreateExpenseClaimLineInput } from "../ports/expense-claim-repo.js";
...
export class DrizzleExpenseClaimRepo implements IExpenseClaimRepo {
  ...
  async findById(id: string): Promise<ExpenseClaim | null> {
    const rows = await this.db.select().from(expenseClaims).where(eq(expenseClaims.id, id)).limit(1);
    return rows[0] ? mapClaimToDomain(rows[0]) : null;
  }

  async findByEmployee(employeeId: string): Promise<readonly ExpenseClaim[]> {
    const rows = await this.db.select().from(expenseClaims).where(eq(expenseClaims.employeeId, employeeId));
  ...
  async create(tenantId: string, input: CreateExpenseClaimInput): Promise<ExpenseClaim> {
    const [row] = await this.db.insert(expenseClaims).values({
      tenantId,
  ...
  async update(id: string, input: Partial<Record<string, unknown>>): Promise<ExpenseClaim> {
    const [row] = await this.db.update(expenseClaims).set(input).where(eq(expenseClaims.id, id)).returning();
    return mapClaimToDomain(row!);
  }

  async findLinesByClaim(claimId: string): Promise<readonly ExpenseClaimLine[]> {
    const rows = await this.db.select().from(expenseClaimLines).where(eq(expenseClaimLines.claimId, claimId));
    return rows.map(mapLineToDomain);
  }

  async createLine(tenantId: string, input: CreateExpenseClaimLineInput): Promise<ExpenseClaimLine> {
    const [row] = await this.db.insert(expenseClaimLines).values({
      tenantId,
