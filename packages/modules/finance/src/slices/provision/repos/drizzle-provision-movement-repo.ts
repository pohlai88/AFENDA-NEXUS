...
import type { IProvisionMovementRepo, CreateProvisionMovementInput } from "../ports/provision-movement-repo.js";
...
export class DrizzleProvisionMovementRepo implements IProvisionMovementRepo {
  ...
  async findByProvision(provisionId: string): Promise<readonly ProvisionMovement[]> {
    const rows = await this.db.select().from(provisionMovements).where(eq(provisionMovements.provisionId, provisionId));
    return rows.map(mapToDomain);
  }

  async create(tenantId: string, input: CreateProvisionMovementInput): Promise<ProvisionMovement> {
    const [row] = await this.db.insert(provisionMovements).values({ tenantId, ...input }).returning();
