...
export function registerProvisionRoutes(app: FastifyInstance, runtime: FinanceRuntime): void {
  app.get("/provisions", async (req) => {
  ...
  app.get<{ Params: { id: string } }>("/provisions/:id", async (req, reply) => {
    const tenantId = (req.headers as Record<string, string>)["x-tenant-id"]!;
    const userId = (req.headers as Record<string, string>)["x-user-id"]!;
    return runtime.withTenant({ tenantId, userId }, async (deps) => {
  ...
  app.get<{ Params: { id: string } }>("/provisions/:id/movements", async (req) => {
    const tenantId = (req.headers as Record<string, string>)["x-tenant-id"]!;
    const userId = (req.headers as Record<string, string>)["x-user-id"]!;
    return runtime.withTenant({ tenantId, userId }, async (deps) => {
