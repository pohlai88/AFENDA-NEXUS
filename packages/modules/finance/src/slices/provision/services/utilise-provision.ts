...
export async function utiliseProvision(
  ...
  deps: { provisionRepo: IProvisionRepo; provisionMovementRepo: IProvisionMovementRepo; outboxWriter: IOutboxWriter },
  ...
  const balanceAfter = input.movementType === "UTILISATION"
  ...
  const newStatus = balanceAfter <= 0n
    ? (input.movementType === "REVERSAL" ? "REVERSED" as const : "FULLY_UTILISED" as const)
    : "PARTIALLY_UTILISED" as const;
  await deps.provisionRepo.updateStatus(input.provisionId, newStatus);

  const eventType = input.movementType === "UTILISATION"
