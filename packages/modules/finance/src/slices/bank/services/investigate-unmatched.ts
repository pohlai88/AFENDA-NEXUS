...
export async function investigateUnmatched(
  ...
  if (lines.length === 0)
    return err(new ValidationError('No unmatched line found or line already matched'));

  const line = lines[0]!;
  if (line.matchStatus !== 'UNMATCHED')
    return err(new ValidationError('Line is not in UNMATCHED status'));

  const updated = await deps.bankStatementRepo.updateLineMatchStatus(
