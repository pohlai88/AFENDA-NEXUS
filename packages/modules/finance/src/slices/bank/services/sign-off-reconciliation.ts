...
export async function signOffReconciliation(
  ...
  if (recon.difference !== 0n) return err(new ValidationError("Cannot sign off with non-zero difference"));
  if (recon.unmatchedCount > 0) return err(new ValidationError("Cannot sign off with unmatched items"));
  ...
  await deps.outboxWriter.write({
    ...
    payload: { reconciliationId: signed.id, bankAccountId: signed.bankAccountId, userId: input.userId },
  });
