...
import type { IBankStatementRepo, CreateBankStatementInput, CreateStatementLineInput } from "../ports/bank-statement-repo.js";
...
export class DrizzleBankStatementRepo implements IBankStatementRepo {
  ...
  async findById(id: string): Promise<BankStatement | null> {
    const rows = await this.db.select().from(bankStatements).where(eq(bankStatements.id, id)).limit(1);
    return rows[0] ? mapStmtToDomain(rows[0]) : null;
  }

  async findByBankAccount(bankAccountId: string): Promise<readonly BankStatement[]> {
    const rows = await this.db.select().from(bankStatements).where(eq(bankStatements.bankAccountId, bankAccountId));
    return rows.map(mapStmtToDomain);
  }

  async create(tenantId: string, input: CreateBankStatementInput): Promise<BankStatement> {
    const [row] = await this.db.insert(bankStatements).values({
      tenantId,
  ...
  async findLinesByStatement(statementId: string): Promise<readonly BankStatementLine[]> {
    const rows = await this.db.select().from(bankStatementLines).where(eq(bankStatementLines.statementId, statementId));
  ...
  async createLine(tenantId: string, input: CreateStatementLineInput): Promise<BankStatementLine> {
    const [row] = await this.db.insert(bankStatementLines).values({
      tenantId,
  ...
  async updateLineMatchStatus(lineId: string, status: BankStatementLine["matchStatus"], matchId: string | null): Promise<BankStatementLine> {
    const [row] = await this.db.update(bankStatementLines).set({ matchStatus: status, matchId }).where(eq(bankStatementLines.id, lineId)).returning();
    return mapLineToDomain(row!);
  }
}
