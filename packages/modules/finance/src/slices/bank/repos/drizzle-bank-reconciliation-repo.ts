...
import type { IBankReconciliationRepo, CreateBankReconciliationInput } from "../ports/bank-reconciliation-repo.js";
...
export class DrizzleBankReconciliationRepo implements IBankReconciliationRepo {
  ...
  async findById(id: string): Promise<BankReconciliation | null> {
    const rows = await this.db.select().from(bankReconciliations).where(eq(bankReconciliations.id, id)).limit(1);
    return rows[0] ? mapToDomain(rows[0]) : null;
  }

  async findByBankAccount(bankAccountId: string): Promise<readonly BankReconciliation[]> {
    const rows = await this.db.select().from(bankReconciliations).where(eq(bankReconciliations.bankAccountId, bankAccountId));
  ...
  async create(tenantId: string, input: CreateBankReconciliationInput): Promise<BankReconciliation> {
  ...
  async signOff(id: string, signedOffBy: string): Promise<BankReconciliation> {
    const [row] = await this.db.update(bankReconciliations).set({
      status: "SIGNED_OFF",
      signedOffAt: new Date(),
      signedOffBy,
    }).where(eq(bankReconciliations.id, id)).returning();
    return mapToDomain(row!);
  }
}
