...
export function registerBankRoutes(app: FastifyInstance, runtime: FinanceRuntime): void {
  app.get("/bank-statements", async (req) => {
    ...
    return runtime.withTenant({ tenantId, userId }, async (deps) => {
      if (bankAccountId) return { data: await deps.bankStatementRepo.findByBankAccount(bankAccountId) };
      return { data: [] };
    });
  });

  app.get<{ Params: { id: string } }>("/bank-statements/:id", async (req, reply) => {
    const tenantId = (req.headers as Record<string, string>)["x-tenant-id"]!;
    const userId = (req.headers as Record<string, string>)["x-user-id"]!;
    return runtime.withTenant({ tenantId, userId }, async (deps) => {
  ...
  app.get<{ Params: { id: string } }>("/bank-statements/:id/lines", async (req) => {
    const tenantId = (req.headers as Record<string, string>)["x-tenant-id"]!;
    const userId = (req.headers as Record<string, string>)["x-user-id"]!;
    return runtime.withTenant({ tenantId, userId }, async (deps) => {
  ...
  app.get("/bank-reconciliations", async (req) => {
    const tenantId = (req.headers as Record<string, string>)["x-tenant-id"]!;
    const userId = (req.headers as Record<string, string>)["x-user-id"]!;
    return runtime.withTenant({ tenantId, userId }, async (deps) => {
  ...
  app.get<{ Params: { id: string } }>("/bank-reconciliations/:id", async (req, reply) => {
    const tenantId = (req.headers as Record<string, string>)["x-tenant-id"]!;
    const userId = (req.headers as Record<string, string>)["x-user-id"]!;
    return runtime.withTenant({ tenantId, userId }, async (deps) => {
  ...
  app.post<{ Params: { id: string } }>("/bank-reconciliations/:id/sign-off", async (req, reply) => {
    const tenantId = (req.headers as Record<string, string>)["x-tenant-id"]!;
    const userId = (req.headers as Record<string, string>)["x-user-id"]!;
    return runtime.withTenant({ tenantId, userId }, async (deps) => {
      const result = await signOffReconciliation({ tenantId, userId, reconciliationId: req.params.id }, deps);
