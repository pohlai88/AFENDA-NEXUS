...
import { detectExceptions, type JournalEntryInput } from '../calculators/exception-reporting.js';
import {
  reviewUserAccess,
...
describe('detectExceptions', () => {
  ...
  it('detects amount exceeding threshold', () => {
    const { result } = detectExceptions([{ ...baseEntry, totalAmount: 20_000_000_00n }]);
    const found = result.exceptions.filter((e) => e.exceptionType === 'AMOUNT_EXCEEDS_THRESHOLD');
  ...
  it('detects weekend posting', () => {
    ...
    const found = result.exceptions.filter((e) => e.exceptionType === 'WEEKEND_POSTING');
    expect(found.length).toBe(1);
  });

  it('detects round number bias', () => {
    const { result } = detectExceptions([{ ...baseEntry, totalAmount: 5_000_000_00n }]);
    const found = result.exceptions.filter((e) => e.exceptionType === 'ROUND_NUMBER_BIAS');
    expect(found.length).toBe(1);
  });
  ...
  it('detects duplicate amounts across journals', () => {
    ...
    const found = result.exceptions.filter((e) => e.exceptionType === 'DUPLICATE_AMOUNT');
  ...
  it('detects period-end clustering', () => {
    ...
    const found = result.exceptions.filter((e) => e.exceptionType === 'PERIOD_END_CLUSTERING');
    expect(found.length).toBe(1);
  });
  ...
  it('detects single-line dominance', () => {
    ...
    const found = result.exceptions.filter((e) => e.exceptionType === 'SINGLE_LINE_DOMINANCE');
    expect(found.length).toBe(1);
  });

  it('respects custom config thresholds', () => {
    const { result } = detectExceptions([{ ...baseEntry, totalAmount: 1_000_00n }], {
...
describe('reviewUserAccess', () => {
  ...
  it('detects orphaned account (inactive with roles)', () => {
    ...
    const found = result.findings.filter((f) => f.finding === 'ORPHANED_ACCOUNT');
    expect(found.length).toBe(1);
  });
  ...
  it('detects role accumulation', () => {
    ...
    const found = result.findings.filter((f) => f.finding === 'ROLE_ACCUMULATION');
    expect(found.length).toBe(1);
  });
  ...
  it('detects excessive privilege', () => {
    ...
    const found = result.findings.filter((f) => f.finding === 'EXCESSIVE_PRIVILEGE');
    expect(found.length).toBe(1);
  });
  ...
  it('detects missing approval for privileged role', () => {
    ...
    const found = result.findings.filter((f) => f.finding === 'MISSING_APPROVAL');
    expect(found.length).toBe(1);
  });
