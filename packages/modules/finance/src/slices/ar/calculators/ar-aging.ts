...
export function computeArAging(invoices: readonly ArInvoice[], asOfDate: Date): ArAgingReport {
  ...
  for (const inv of open) {
    ...
    if (daysOverdue <= 0) {
      row = {
        ...existing,
        current: existing.current + outstanding,
        total: existing.total + outstanding,
      };
    } else if (daysOverdue <= 30) {
      row = {
        ...existing,
        days30: existing.days30 + outstanding,
        total: existing.total + outstanding,
      };
    } else if (daysOverdue <= 60) {
      row = {
        ...existing,
        days60: existing.days60 + outstanding,
        total: existing.total + outstanding,
      };
    } else if (daysOverdue <= 90) {
      row = {
        ...existing,
        days90: existing.days90 + outstanding,
        total: existing.total + outstanding,
      };
    } else {
      row = {
        ...existing,
        over90: existing.over90 + outstanding,
        total: existing.total + outstanding,
      };
    }
