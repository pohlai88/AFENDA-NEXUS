...
export class DrizzleArInvoiceRepo implements IArInvoiceRepo {
  ...
  async create(input: CreateArInvoiceInput): Promise<Result<ArInvoice>> {
    ...
    const [row] = await this.tx.insert(arInvoices).values({
    ...
    for (let i = 0; i < input.lines.length; i++) {
      ...
      const [lineRow] = await this.tx.insert(arInvoiceLines).values({
        tenantId: input.tenantId,
        invoiceId: row.id,
        lineNumber: i + 1,
  ...
  async findByCustomer(customerId: string, params?: PaginationParams): Promise<PaginatedResult<ArInvoice>> {
    ...
    const [rows, [{ total }]] = await Promise.all([
      ...
      this.tx.select({ total: count() }).from(arInvoices).where(eq(arInvoices.customerId, customerId)),
    ]);

    const data = await Promise.all(rows.map(async (r) => {
      const lines = await this.tx.query.arInvoiceLines.findMany({ where: eq(arInvoiceLines.invoiceId, r.id) });
  ...
  async findByStatus(status: string, params?: PaginationParams): Promise<PaginatedResult<ArInvoice>> {
    const page = params?.page ?? 1;
    const limit = params?.limit ?? 20;
    const offset = (page - 1) * limit;
    ...
    const [rows, [{ total }]] = await Promise.all([
      ...
      this.tx.select({ total: count() }).from(arInvoices).where(eq(arInvoices.status, status as typeof arInvoices.$inferSelect.status)),
    ]);
  ...
  async recordPayment(id: string, amount: bigint): Promise<Result<ArInvoice>> {
    await this.tx.update(arInvoices).set({
    ...
    if (row && row.paidAmount >= row.totalAmount) {
      await this.tx.update(arInvoices).set({ status: "PAID", updatedAt: new Date() }).where(eq(arInvoices.id, id));
    } else if (row && row.paidAmount > 0n) {
      await this.tx.update(arInvoices).set({ status: "PARTIALLY_PAID", updatedAt: new Date() }).where(eq(arInvoices.id, id));
    }
