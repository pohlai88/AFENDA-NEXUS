...
describe('computeRollingForecast', () => {
  ...
  it('computes growth-rate forecast', () => {
    const result = computeRollingForecast({
      historicalPeriods: [{ periodId: 'p1', accountCode: '6000', actualAmount: 10000n }],
      method: 'GROWTH_RATE',
      periodsToForecast: 1,
      growthRateBps: 500, // 5%
      currencyCode: 'USD',
...
describe('computeEncumbrance', () => {
      ...
      encumbrances: [
        {
          ...
          accountCode: '6100',
          encumberedAmount: 5000n,
          liquidatedAmount: 2000n,
        },
      ],
      budgetLines: [{ accountCode: '6100', budgetAmount: 20000n, actualSpent: 8000n }],
      currencyCode: 'USD',
    });
      ...
      encumbrances: [
        {
          ...
          documentType: 'PURCHASE_ORDER',
          accountCode: '6100',
          encumberedAmount: 15000n,
          liquidatedAmount: 0n,
        },
      ],
      budgetLines: [{ accountCode: '6100', budgetAmount: 20000n, actualSpent: 10000n }],
      currencyCode: 'USD',
    });
    const acct = result.result.accounts[0]!;
  ...
  it('handles accounts with no encumbrances', () => {
    const result = computeEncumbrance({
      encumbrances: [],
      budgetLines: [{ accountCode: '6100', budgetAmount: 20000n, actualSpent: 5000n }],
      currencyCode: 'USD',
    });
...
describe('computeScenarioPlanning', () => {
  ...
  it('applies global adjustment when no per-account override', () => {
    const result = computeScenarioPlanning({
      baseline: [{ accountCode: '4000', accountName: 'Revenue', baselineAmount: 100000n }],
      scenarios: [
  ...
  it('produces summaries per scenario', () => {
    const result = computeScenarioPlanning({
      baseline: [{ accountCode: '4000', accountName: 'Rev', baselineAmount: 50000n }],
      scenarios: [
  ...
  it('throws on empty baseline', () => {
    expect(() =>
      computeScenarioPlanning({
        baseline: [],
        scenarios: [{ scenarioId: 'a', name: 'A', adjustments: [] }],
        currencyCode: 'USD',
      })
...
describe('computeBudgetConsolidation', () => {
  ...
  it('applies inter-unit eliminations', () => {
    const result = computeBudgetConsolidation({
      ...
      eliminations: [{ accountCode: 'ic-rev', sourceId: 'div-a', counterpartSourceId: 'div-b' }],
      targetCurrencyCode: 'USD',
    });
