...
describe('deriveMultiLedger', () => {
  ...
  it('evaluates conditions (EQ)', () => {
    ...
    const noMatch = deriveMultiLedger([makeEvent({ sourceSystem: 'ERP' })], [rule]);
    expect(noMatch.result.derivedLines).toHaveLength(0);
  });

  it('throws on empty events', () => {
    expect(() => deriveMultiLedger([], [makeRule()])).toThrow('At least one accounting event');
  });
});
...
describe('previewDerivation', () => {
  ...
  it('warns about unmatched events', () => {
    const result = previewDerivation([makeEvent({ eventType: 'UNKNOWN' })], [makeRule()]);
  ...
  it('reports zero lines for non-matching', () => {
    const result = previewDerivation([makeEvent({ eventType: 'OTHER' })], [makeRule()]);
