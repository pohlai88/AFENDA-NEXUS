...
import type { ICostAllocationRunRepo, CreateAllocationRunInput, CreateAllocationLineInput } from "../ports/cost-allocation-run-repo.js";
...
export class DrizzleCostAllocationRunRepo implements ICostAllocationRunRepo {
  ...
  async findById(id: string): Promise<CostAllocationRun | null> {
    const rows = await this.db.select().from(costAllocationRuns).where(eq(costAllocationRuns.id, id)).limit(1);
  ...
  async findByPeriod(companyId: string, periodId: string): Promise<readonly CostAllocationRun[]> {
    const rows = await this.db.select().from(costAllocationRuns)
      .where(and(eq(costAllocationRuns.companyId, companyId), eq(costAllocationRuns.periodId, periodId)));
  ...
  async create(tenantId: string, input: CreateAllocationRunInput): Promise<CostAllocationRun> {
    const [row] = await this.db.insert(costAllocationRuns).values({
      tenantId,
  ...
  async updateStatus(id: string, status: CostAllocationRun["status"], totalAllocated: bigint, lineCount: number): Promise<CostAllocationRun> {
    ...
    const [row] = await this.db.update(costAllocationRuns).set(updates).where(eq(costAllocationRuns.id, id)).returning();
    return mapRunToDomain(row!);
  }

  async createLines(tenantId: string, lines: readonly CreateAllocationLineInput[]): Promise<readonly CostAllocationLine[]> {
    if (lines.length === 0) return [];
    const rows = await this.db.insert(costAllocationLines).values(
      lines.map((l) => ({
        tenantId,
        runId: l.runId,
  ...
  async findLinesByRun(runId: string): Promise<readonly CostAllocationLine[]> {
    const rows = await this.db.select().from(costAllocationLines).where(eq(costAllocationLines.runId, runId));
    return rows.map(mapLineToDomain);
  }
}
