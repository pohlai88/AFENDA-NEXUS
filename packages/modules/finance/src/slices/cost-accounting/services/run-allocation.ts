...
export async function runCostAllocation(
  ...
  try {
    ...
    if (input.method !== "DIRECT") {
      await deps.costAllocationRunRepo.updateStatus(run.id, "FAILED", 0n, 0);
      return err(new ValidationError(`Method ${input.method} requires additional configuration — use direct allocation or provide step-down/reciprocal config`));
    }
    ...
    if (pools.length === 0 || targets.length === 0) {
      ...
      return { ok: true, value: await deps.costAllocationRunRepo.findById(run.id) as CostAllocationRun };
    }
    ...
    const completed = await deps.costAllocationRunRepo.updateStatus(
      run.id, "COMPLETED", result.totalAllocated, lines.length,
    );
