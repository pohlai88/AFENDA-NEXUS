...
describe('computeStepDownAllocation', () => {
  it('allocates in sequence order', () => {
    ...
    const adminToIt = result.lines.find(
      (l) => l.fromCostCenterId === 'admin' && l.toCostCenterId === 'it'
    );
  ...
  it('throws on empty activities', () => {
    expect(() =>
      computeActivityBasedCosting({
        activities: [],
        costObjects: [{ objectId: 'a', name: 'A', activityConsumption: [] }],
        currencyCode: 'USD',
      })
  ...
  it('throws on zero units produced', () => {
    expect(() =>
      computeStandardCostVariance({
        ...
        unitsProduced: 0n,
        material: {
          standardPricePerUnit: 1n,
          standardQtyPerUnit: 1n,
          actualPricePerUnit: 1n,
          actualQtyUsed: 1n,
        },
        labor: {
          standardRatePerHour: 1n,
          standardHoursPerUnit: 1n,
          actualRatePerHour: 1n,
          actualHoursWorked: 1n,
        },
...
describe('computeOverheadAbsorption', () => {
  it('computes over/under absorption', () => {
    const result = computeOverheadAbsorption({
      budgets: [
        {
          costCenterId: 'factory',
          budgetedOverhead: 100000n,
          budgetedBasisQuantity: 10000n,
          absorptionBasis: 'MACHINE_HOURS',
          currencyCode: 'USD',
        },
      ],
      actuals: [{ costCenterId: 'factory', actualBasisQuantity: 9500n, actualOverhead: 98000n }],
    });
    expect(result.lines).toHaveLength(1);
      ...
      budgets: [
        {
          ...
          budgetedOverhead: 100000n,
          budgetedBasisQuantity: 10000n,
          absorptionBasis: 'DIRECT_LABOR_HOURS',
          currencyCode: 'USD',
        },
      ],
      actuals: [{ costCenterId: 'factory', actualBasisQuantity: 11000n, actualOverhead: 105000n }],
    });
  ...
  it('throws on empty budgets', () => {
    expect(() => computeOverheadAbsorption({ budgets: [], actuals: [] })).toThrow(
      'At least one overhead budget'
    );
  });
});
...
describe('computeProfitability', () => {
      ...
      costItems: [
        {
          ...
          directCost: 60000n,
          allocatedOverhead: 10000n,
        },
        {
          dimensionId: 'prod-b',
          dimensionType: 'PRODUCT',
          directCost: 35000n,
          allocatedOverhead: 8000n,
        },
      ],
      currencyCode: 'USD',
    });
  ...
  it('handles dimension with only costs (no revenue)', () => {
    const result = computeProfitability({
      ...
      costItems: [
        {
          dimensionId: 'overhead',
          dimensionType: 'SEGMENT',
          directCost: 5000n,
          allocatedOverhead: 2000n,
        },
      ],
      currencyCode: 'USD',
    });
