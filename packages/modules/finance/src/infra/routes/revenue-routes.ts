...
import { IdParamSchema, CreateRevenueContractSchema, RecognizeRevenueSchema, PaginationSchema } from "@afenda/contracts";
...
export function registerRevenueRoutes(
  ...
  app.post("/revenue-contracts", async (req, reply) => {
  ...
  app.get("/revenue-contracts", async (req, reply) => {
    const tenantId = req.headers["x-tenant-id"] as string;
  ...
  app.get("/revenue-contracts/:id", async (req, reply) => {
  ...
  app.get("/revenue-contracts/:id/milestones", async (req, reply) => {
    const { id } = IdParamSchema.parse(req.params);
    const tenantId = req.headers["x-tenant-id"] as string;
    const userId = (req.headers["x-user-id"] as string) ?? "system";
  ...
  app.post("/revenue-contracts/:id/recognize", async (req, reply) => {
    const { id } = IdParamSchema.parse(req.params);
    const tenantId = req.headers["x-tenant-id"] as string;
    const userId = req.headers["x-user-id"] as string;
