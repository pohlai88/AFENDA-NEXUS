...
export async function seed(connectionString: string): Promise<void> {
  ...
  try {
    ...
    const [insertedUser] = await db
      ...
      .returning();
    const adminUser =
      insertedUser ?? (await db.select().from(users).where(eq(users.tenantId, tid)).limit(1))[0];
    ...
    await db
      ...
      .onConflictDoNothing();
    const [hqLedger] = await db
      .select()
      .from(ledgers)
      .where(and(eq(ledgers.tenantId, tid), eq(ledgers.companyId, hqCompany.id)))
      .limit(1);
    ...
    const [journal] = await db
      .select()
      .from(glJournals)
      .where(eq(glJournals.tenantId, tid))
      .limit(1);
