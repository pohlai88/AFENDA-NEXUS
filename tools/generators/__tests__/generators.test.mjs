...
const TEST_SPEC = {
  ...
  commands: [
    ...
    {
      name: 'suspend',
      service: 'suspendArCustomer',
      route: 'POST /ar/customers/:id/suspend',
      idempotent: true,
    },
  ],
  frontend: {
...
describe('pluralize', () => {
  it.each([
    ...
    ['Delay', 'Delays'], // vowel+y → just add s
...
describe('patchEmptyStateRegistry', () => {
  let tmpRoot;

  beforeAll(() => {
    tmpRoot = join(tmpdir(), `afenda-es-test-${Date.now()}`);
    const erpDir = join(tmpRoot, 'apps', 'web', 'src', 'components', 'erp');
    mkdirSync(erpDir, { recursive: true });

    // Create mock generated-keys file with anchored region
    writeFileSync(
      join(erpDir, 'empty-state.generated-keys.ts'),
    ...
    // Create mock registry file with anchored region
    writeFileSync(
      join(erpDir, 'empty-state.registry.ts'),
  ...
  it('inserts key and registry entry', () => {
    const result = patchEmptyStateRegistry({
      registryKey: 'finance.testEntity',
      displayName: 'Test Entity',
      displayPlural: 'Test Entities',
      description: 'Track your test entities here.',
      root: tmpRoot,
    });

    expect(result.keysPatched).toBe(true);
    expect(result.registryPatched).toBe(true);

    const erpDir = join(tmpRoot, 'apps', 'web', 'src', 'components', 'erp');
    const keysContent = readFileSync(join(erpDir, 'empty-state.generated-keys.ts'), 'utf-8');
    // Prettier may reformat the union pipe syntax, but key string must be present
    expect(keysContent).toContain('finance.testEntity');

    const regContent = readFileSync(join(erpDir, 'empty-state.registry.ts'), 'utf-8');
    expect(regContent).toContain('finance.testEntity');
    expect(regContent).toContain('No test entities yet');
    expect(regContent).toContain('Track your test entities here.');
    expect(regContent).toContain('Create Test Entity');
  });
  ...
  it('is idempotent (second run is a no-op)', () => {
    ...
    // With dedupKey fix, patchAnchoredRegion checks for the key string
    // rather than the full insertion text, making it formatter-resilient.
    expect(result.keysPatched).toBe(false);
    expect(result.registryPatched).toBe(false);

    // Verify no duplicates
    const erpDir = join(tmpRoot, 'apps', 'web', 'src', 'components', 'erp');
    const keysContent = readFileSync(join(erpDir, 'empty-state.generated-keys.ts'), 'utf-8');
    const matches = keysContent.match(/finance\.testEntity/g);
    expect(matches).toHaveLength(1);
  });

  it('inserts a second key without overwriting the first', () => {
    const result = patchEmptyStateRegistry({
      registryKey: 'finance.secondEntity',
      displayName: 'Second Entity',
      displayPlural: 'Second Entities',
      root: tmpRoot,
    });
    ...
    const keysContent = readFileSync(join(erpDir, 'empty-state.generated-keys.ts'), 'utf-8');
    expect(keysContent).toContain('finance.testEntity');
    expect(keysContent).toContain('finance.secondEntity');
  });
});
