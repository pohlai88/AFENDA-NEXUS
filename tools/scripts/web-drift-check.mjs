...
function checkW04() {
  const SUSPECT_PATTERN = /(?:interface|type)\s+\w+(?:Payload|Request|Response|Body|Input|Output)\b/;
...
function checkW07() {
  const uiDir = join(SRC, "components/ui");
  if (!existsSync(uiDir)) {
    warn("W07", "components/ui/ directory is empty or missing. Run `npx shadcn@latest add` to populate.");
    return;
  }
...
function checkW11() {
  ...
  // Check queries/ files
  const queryFiles = collectFiles(SRC, [".ts"]).filter((f) => f.replace(/\\/g, "/").includes("/queries/"));
...
function checkW13() {
  ...
  for (const dep of Object.keys(devDeps)) {
    ...
    if (ALLOWED_DEV.includes(dep)) {
      ...
      fail("W13", `DevDep "${dep}" NOT in ARCHITECTURE dev allowlist. Add to ARCHITECTURE.md or remove.`);
    }
  }
  ...
  if (twVersion && !twVersion.includes("catalog:")) {
    if (!twVersion.startsWith("^4") && !twVersion.startsWith("4") && !twVersion.startsWith("~4")) {
      fail("W13", `tailwindcss version "${twVersion}" is not v4. Architecture requires Tailwind CSS v4.`);
    }
  }
...
function checkW14() {
  ...
  const HARDCODED_COLOR_PATTERN = /(?:bg|text|border|ring|outline|shadow|from|to|via)-(?:red|green|yellow|orange|purple|pink|indigo|violet|teal|cyan|lime|amber|fuchsia|rose|sky|stone|zinc|gray|slate|neutral)-\d{2,3}/;
