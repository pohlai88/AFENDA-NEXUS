...
<Prerequisites>- Database [connection basics](/docs/connect-overview) with Drizzle</Prerequisites>
...
<Callout>
  Response from the batch should be an array of raw values (an array within an array), in the same
  order as they were sent to the proxy server
</Callout>
