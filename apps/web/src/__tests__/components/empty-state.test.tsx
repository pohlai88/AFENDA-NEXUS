...
import { getEmptyStateContent } from '@/components/erp/empty-state.registry';
...
describe('EmptyState', () => {
  ...
  it('does not render description when not provided', () => {
    const { container } = renderWithProviders(
      <EmptyState title="No items" animate={false} />,
    );
  ...
  it.each(['firstRun', 'noResults', 'error', 'forbidden'] as const)(
    ...
    (variant) => {
      renderWithProviders(
        <EmptyState title="Test" variant={variant} animate={false} />,
      );
  ...
  it.each(['sm', 'md', 'lg'] as const)('applies size="%s" classes', (size) => {
    renderWithProviders(
      <EmptyState title="Test" size={size} animate={false} />,
    );
  ...
  it('resolves content from registry via contentKey', () => {
    renderWithProviders(
      <EmptyState contentKey="finance.journals" animate={false} />,
    );
    expect(screen.getByText('No journal entries yet')).toBeInTheDocument();
    expect(
      screen.getByText('Create your first journal entry to get started.'),
    ).toBeInTheDocument();
  });
  ...
  it('renders dashed border container', () => {
    const { container } = renderWithProviders(
      <EmptyState title="Empty" animate={false} />,
    );
