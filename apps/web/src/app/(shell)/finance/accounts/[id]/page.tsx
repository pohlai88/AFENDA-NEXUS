    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center gap-6 rounded-md border p-4">
        <div>
          ...
          <div className="mt-1">
            <StatusBadge status={account.isActive ? 'OPEN' : 'CLOSED'} />
          </div>
        </div>
        <Separator orientation="vertical" className="hidden h-10 sm:block" />
        <div>
          <span className="text-xs text-muted-foreground">Account Code</span>
          <div className="mt-1 font-mono text-sm font-medium">{account.code}</div>
        </div>
        <Separator orientation="vertical" className="hidden h-10 sm:block" />
        <div>
        ...
        <div>
          ...
          <div className="mt-1">
            <Badge variant="secondary">{account.normalBalance}</Badge>
          </div>
        </div>
        {account.parentId && (
          <>
            ...
            <div>
              ...
              <div className="mt-1 text-sm font-medium">{account.parentId}</div>
            </div>
          </>
        )}
      </div>
