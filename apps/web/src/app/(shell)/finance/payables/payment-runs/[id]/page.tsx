    ...
    <div className="space-y-6">
      ...
      <Tabs defaultValue="items">
          ...
          {run.items.length > 0 ? (
            ...
            <Card>
              <CardContent className="py-8 text-center text-sm text-muted-foreground">
                No items added yet. Add invoices to this payment run.
              </CardContent>
            </Card>
          )}
        </TabsContent>
        <TabsContent value="report" className="mt-4">
          <Card>
            <CardHeader>
              <CardTitle className="text-base">Payment Run Report</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="grid grid-cols-2 gap-4 md:grid-cols-4 text-sm">
                <div>
                  <p className="text-xs text-muted-foreground">Total Gross</p>
                  <p className="font-medium tabular-nums">{run.totalAmount} {run.currencyCode}</p>
                </div>
                <div>
                  <p className="text-xs text-muted-foreground">Total Discount</p>
                  <p className="font-medium tabular-nums">{run.totalDiscount} {run.currencyCode}</p>
                </div>
                <div>
                  <p className="text-xs text-muted-foreground">Total Net</p>
                  <p className="font-medium tabular-nums">{run.totalNet} {run.currencyCode}</p>
                </div>
                <div>
                  <p className="text-xs text-muted-foreground">Invoices</p>
                  <p className="font-medium tabular-nums">{run.itemCount}</p>
                </div>
              </div>
            </CardContent>
          </Card>
