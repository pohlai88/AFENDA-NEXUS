...
async function getTaxSummaryData(): Promise<TaxSummaryData> {
  ...
  return {
    ...
    inputTax: 625000,
    netPayable: 225000,
    withholdingTax: 45000,
    corporateTax: 1250000,
    deferredTax: 100000,
    provisions: 85000,
    currency: 'USD',
    period: 'Q1 2026',
...
async function TaxSummaryContent() {
  ...
  return (
    <div className="grid gap-6 md:grid-cols-2">
      <Card>
        <CardHeader>
          <CardTitle>VAT/GST Summary</CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="flex justify-between items-center py-2 border-b">
            <span>Output Tax (Sales)</span>
            <span className="font-mono font-semibold">
              {formatCurrency(data.outputTax, data.currency)}
            </span>
          </div>
          <div className="flex justify-between items-center py-2 border-b">
            <span>Input Tax (Purchases)</span>
            <span className="font-mono font-semibold text-green-600">
              ({formatCurrency(data.inputTax, data.currency)})
            </span>
          </div>
          <div className="flex justify-between items-center py-2 bg-muted/50 rounded px-2">
            ...
            <span className="font-mono font-bold text-lg">
              {formatCurrency(data.netPayable, data.currency)}
            </span>
          </div>
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent>
          <div className="flex justify-between items-center py-2 border-b">
            ...
            <span className="font-mono font-semibold">
              {formatCurrency(data.withholdingTax, data.currency)}
            </span>
          </div>
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div className="flex justify-between items-center py-2 border-b">
            ...
            <span className="font-mono font-semibold">
              {formatCurrency(data.corporateTax, data.currency)}
            </span>
          </div>
          <div className="flex justify-between items-center py-2 border-b">
            <span>Deferred Tax Movement</span>
            <span className="font-mono font-semibold">
              {formatCurrency(data.deferredTax, data.currency)}
            </span>
          </div>
          <div className="flex justify-between items-center py-2 bg-muted/50 rounded px-2">
            <span className="font-semibold">Total Tax Expense</span>
            <span className="font-mono font-bold">
              {formatCurrency(data.corporateTax + data.deferredTax, data.currency)}
            </span>
          </div>
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Tax Provisions</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="flex justify-between items-center py-2 border-b">
            <span>Uncertain Tax Positions</span>
            <span className="font-mono font-semibold">
              {formatCurrency(data.provisions, data.currency)}
            </span>
          </div>
        </CardContent>
      </Card>
...
export default function TaxSummaryPage() {
  return (
    <ReportWrapper
      ...
      description="Consolidated view of all tax obligations"
