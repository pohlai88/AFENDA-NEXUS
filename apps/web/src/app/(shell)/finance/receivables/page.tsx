...
import { ArInvoiceTable } from '@/features/finance/receivables/blocks/ar-invoice-table';
...
export default async function ReceivablesPage({ searchParams }: ReceivablesPageProps) {
  ...
  const result = await getArInvoices(ctx, {
  ...
  if (!result.ok) {
    handleApiError(result, 'Failed to load receivable invoices');
  }
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Filter bar */}
      <ArListFilters currentStatus={params.status} currentPage={page} totalPages={totalPages} />
