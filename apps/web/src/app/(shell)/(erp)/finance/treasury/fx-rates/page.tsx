...
interface FxRatesPageProps {
  searchParams: Promise<{ from?: string; to?: string; date?: string; page?: string; limit?: string }>;
}
    ...
    <div className="space-y-6">
      ...
      <details className="rounded-md border p-4">
        <summary className="flex cursor-pointer items-center gap-2 text-sm font-medium">
          <Plus className="h-4 w-4" />Add FX Rate
        </summary>
        <div className="mt-4"><FxRateCreateForm /></div>
      </details>

      {rates.length === 0 ? (
        <EmptyState contentKey="finance.fxRates" icon={ArrowRightLeft} />
      ) : (
