...
async function ReconcileContent({ ctx, id }: { ctx: RequestContext; id: string }) {
  const [statementResult, sessionResult, transactionsResult, glResult, suggestionsResult] =
    await Promise.all([
      getBankStatementById(ctx, id),
      getReconciliationSession(ctx, id),
      getBankTransactions(ctx, { statementId: id }),
      getGLTransactions(ctx, { bankAccountId: 'ba-1', unreconciledOnly: true }),
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <PageHeaderDescription>
                Statement period: {formatDate(stmt.startDate)} - {formatDate(stmt.endDate)}
                {' \u2022 '}Closing balance: {formatCurrency(stmt.closingBalance, stmt.currency)}
              </PageHeaderDescription>
            </div>
          </div>
        </div>
...
export default async function ReconcilePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <Suspense fallback={<LoadingSkeleton />}>
      <ReconcileContent ctx={ctx} id={id} />
    </Suspense>
  );
}
