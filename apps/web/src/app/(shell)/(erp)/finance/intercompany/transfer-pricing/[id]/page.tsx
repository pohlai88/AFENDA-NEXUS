...
import { KPI, BenchmarksList } from '@/features/finance/transfer-pricing/blocks/policy-detail-blocks';
    ...
    <>
      ...
      <BusinessDocument>
        ...
        <div className="grid gap-4 sm:grid-cols-4 mb-6">
          ...
          <KPI label="Effective From" value={policy.effectiveFrom ? new Date(policy.effectiveFrom).toLocaleDateString() : '—'} />
        </div>
        <Tabs defaultValue="overview">
          ...
          <TabsContent value="overview" className="space-y-4">
            <dl className="grid gap-2 sm:grid-cols-2 text-sm">
              <div><dt className="text-muted-foreground">Entities</dt><dd>{policy.entities?.length ? policy.entities.join(', ') : '—'}</dd></div>
              <div><dt className="text-muted-foreground">Last Reviewed</dt><dd>{policy.lastReviewDate ? new Date(policy.lastReviewDate).toLocaleDateString() : '—'}</dd></div>
              <div><dt className="text-muted-foreground">Created</dt><dd>{new Date(policy.createdAt).toLocaleDateString()}</dd></div>
              <div><dt className="text-muted-foreground">Updated</dt><dd>{new Date(policy.updatedAt).toLocaleDateString()}</dd></div>
            </dl>
          </TabsContent>
