...
export default async function BalanceSheetPage({ searchParams }: BalanceSheetPageProps) {
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Balance Sheet"
        description={data?.asOfDate ? `Assets, liabilities, and equity as of ${data.asOfDate}.` : 'Assets, liabilities, and equity at a point in time.'}
        breadcrumbs={[
      ...
      {(!ledgerId || !periodId) && <EmptyState contentKey="finance.reports.balanceSheet" variant="firstRun" icon={BarChart3} />}

      {data && !data.isBalanced && (
        <div className="rounded-md border border-warning/30 bg-warning/10 p-4 text-sm text-warning" role="alert">
