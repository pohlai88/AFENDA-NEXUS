  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description={data?.asOfDate ? `Intercompany balances aging as of ${data.asOfDate}.` : 'Analyze aging of intercompany balances by counterparty.'}
        breadcrumbs={[
      ...
      <div className="flex flex-wrap items-end gap-4">
        <Suspense>
          <ReportFilterBar variant="currency-date" currencies={filterData.currencies} defaults={{ currency, asOfDate }} />
        </Suspense>
        <Suspense>
      ...
      {(!currency || !asOfDate) && (
        <EmptyState contentKey="finance.reports.icAging" variant="firstRun" icon={ArrowLeftRight} />
      )}

      {data && data.rows.length === 0 && (
        <EmptyState contentKey="finance.reports.icAging" variant="noResults" icon={ArrowLeftRight} />
      )}

      {data && data.rows.length > 0 && (
        <IcAgingTable rows={data.rows.map(r => ({ ...r, current: Number(r.current), days30: Number(r.days30), days60: Number(r.days60), days90Plus: Number(r.days90Plus), total: Number(r.total) }))} currency={data.currency} grandTotal={Number(data.grandTotal)} />
      )}
    </div>
  );
}
