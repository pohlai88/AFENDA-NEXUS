...
async function PaymentRunDetailContent({ ctx, id }: { ctx: RequestContext; id: string }) {
            ...
            <CardContent>
              <div className="grid grid-cols-2 gap-4 md:grid-cols-4 text-sm">
                ...
                <div>
                  ...
                  <p className="font-medium tabular-nums">
                    {run.totalDiscount} {run.currencyCode}
                  </p>
                </div>
                <div>
                  <p className="text-xs text-muted-foreground">Total Net</p>
                  <p className="font-medium tabular-nums">
                    {run.totalNet} {run.currencyCode}
                  </p>
                </div>
                <div>
...
export default async function PaymentRunDetailPage({
  ...
  return (
    <Suspense fallback={<LoadingSkeleton />}>
      <PaymentRunDetailContent ctx={ctx} id={id} />
    </Suspense>
  );
}
