...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: `${c.customerCode} — ${c.customerName} | Credit Management | Finance`,
    description: `Credit limit for ${c.customerCode} — ${c.customerName}`,
  };
}

async function CreditDetailContent({ ctx, id }: { ctx: RequestContext; id: string }) {
  const result = await getCustomerCreditById(ctx, id);
...
export default async function CreditDetailPage({ params }: Props) {
  ...
  return (
    <Suspense fallback={<LoadingSkeleton />}>
      <CreditDetailContent ctx={ctx} id={id} />
    </Suspense>
  );
}
